...
inline void pt_set_page_permissions(uint32* page_directory, uint32 virtual_address, uint32 permissions_to_set, uint32 permissions_to_clear)
{
	...
	int res = get_page_table(page_directory, virtual_address,&pg_table);
	if( res != 0 || pg_table == NULL)
		panic("invalid va");
	pg_table[PTX(virtual_address)]|= permissions_to_set;
	pg_table[PTX(virtual_address)] &= ~permissions_to_clear;
...
inline int pt_get_page_permissions(uint32* page_directory, uint32 virtual_address )
{
	...
	if( res != 0 || pg_table == NULL)
		return -1;
	return pg_table[PTX(virtual_address)] & 0x00000FFF;
}
...
inline void pt_clear_page_table_entry(uint32* page_directory, uint32 virtual_address)
{
	...
	uint32 *pg_table= NULL;
		...
		if( res != 0 || pg_table == NULL)
			panic("invalid va");
		pg_table[PTX(virtual_address)]=0;
		tlb_invalidate((void*)NULL,(void*)virtual_address);
}
...
inline int virtual_to_physical(uint32* page_directory, uint32 virtual_address)
{
	...
	/*uint32 *ptrpage_table;
	struct Frame_Info *frame_info =	get_frame_info(page_directory ,virtual_address , &ptrpage_table);
	//int r =get_page_table(ptr_page_directory,virtual_address,&page_table);
	if(frame_info!=NULL)
	{
	...
	struct FrameInfo *frameinfo= get_frame_info(page_directory , virtual_address , &ptr_pg_table);
	if(frameinfo==NULL)
	{
		return -1;
	}
