...
int createSemaphore(int32 ownerEnvID, char* semaphoreName, uint32 initialValue)
{
	...
	for (int i = 0; i < MAX_SEMAPHORES; ++i)
	{
		if(semaphores[i].ownerID==ownerEnvID)
		{
			 x=1;
		}
	}
	...
	struct Semaphore * allocatedObject;
	 int semaphoreObjectID=-1;
	semaphoreObjectID= allocate_semaphore_object( &allocatedObject);
#endif
#if semaphoreObjectID!=-1
	{
...
void waitSemaphore(int32 ownerEnvID, char* semaphoreName)
{
	...
	semaphores[semaphoreObjectID].env_queue;
	if (semaphores[semaphoreObjectID].value<0)
	{
		enqueue(&semaphores[semaphoreObjectID].env_queue,curenv);
		ENV_BLOCKED;
		curenv=NULL;
	}
	fos_scheduler();
...
void signalSemaphore(int ownerEnvID, char* semaphoreName)
{
	...
	if(semaphores[semaphoreObjectID].value<=0)
	{
	env=dequeue(&semaphores[semaphoreObjectID].env_queue);
